/**
* @Function: Multisensor single-thread estimation thread handle
*
* @Author  : Cheng Chi
...
MultiSensorSingleThreadEstimating::MultiSensorSingleThreadEstimating(
  ...
  EstimatingBase(nodes, i_estimator), latest_imu_timestamp_(0.0)
{
  ...
  if (!enable_input_align_) {
    LOG(WARNING) 
      << "Input align should be enabled in one-thread estimating. Default values setted!";
    enable_input_align_ = true;
    input_align_latency_ = 1.0;
  }
...
// Reset estimator
void MultiSensorSingleThreadEstimating::resetProcessors()
{
...
// Input data callback
void MultiSensorSingleThreadEstimating::estimatorDataCallback(EstimatorDataCluster& data)
{
  ...
  // Handle data according to its property
...
void MultiSensorSingleThreadEstimating::process()
{
  ...
  // Process frontend
  if (estimatorDataNeedFrontend(measurement)) {
    if (!processImageFrontend(measurement, measurement)) return;
  }

  // Process estimator
  processEstimator(measurement);
}

// Update latest solution
bool MultiSensorSingleThreadEstimating::updateSolution()
{
...
void MultiSensorSingleThreadEstimating::handleTimePropagationSensors(EstimatorDataCluster& data)
{
  // only support IMU
...
// Process estimator
bool MultiSensorSingleThreadEstimating::processEstimator(const EstimatorDataCluster& data)
{
  const EstimatorDataCluster& measurement = data;
...
bool MultiSensorSingleThreadEstimating::processImageFrontend(
  const EstimatorDataCluster& data, EstimatorDataCluster& out)
{
  const EstimatorDataCluster& front_measurement = data;
  if (feature_handler_ == nullptr) return false;

  // Process feature detecting and tracking
  const std::shared_ptr<cv::Mat>& image = front_measurement.image;
  ...
  if (ret) {
    ...
    out = EstimatorDataCluster(frame_bundle, tag);

...
class MultiSensorSingleThreadEstimating : public EstimatingBase {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  using Ptr = std::shared_ptr<MultiSensorSingleThreadEstimating>;

  MultiSensorSingleThreadEstimating(const NodeOptionHandlePtr& nodes, size_t i_estimator);
  ~MultiSensorSingleThreadEstimating();
...
private:
  ...
  // Process estimator
  bool processEstimator(const EstimatorDataCluster& data);

  // Process image frontend
  bool processImageFrontend(const EstimatorDataCluster& data, EstimatorDataCluster& out);
